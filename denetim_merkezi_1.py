...
def ensure_lxml():
    try:
        ...
        try:
            ...
            subprocess.check_call([sys.executable, "-m", "pip", "install", "lxml", "html5lib"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        except Exception as e:
            print(f"UYARI: lxml/html5lib otomatik yüklenemedi. Hata: {e}")
ensure_lxml()

# --- BAD CRC-32 (BOZUK EXCEL) BYPASS HACK ---
try:
    from openpyxl.reader.excel import ExcelReader
    ExcelReader.read_properties = lambda self: None
...
try:
    ...
    print(f"UYARI: Bozuk Excel dosyalarını okuma (CRC bypass) yaması uygulanamadı: {e}")
# --------------------------------------------
...
def _repair_excel_file(file_path):
    """
    Bozuk bir Excel (.xlsx) dosyasını onarmaya çalışır.
    Genellikle 'BadZipFile: Bad CRC-32' hatasına neden olan 'docProps' klasörünü siler.
    """
    try:
        # 1. Geçici bir onarım dosyası oluştur
        temp_dir = tempfile.gettempdir()
        repaired_path = os.path.join(temp_dir, f"repaired_{os.path.basename(file_path)}")

        # 2. Orijinal dosyayı geçici dosyaya kopyala
        shutil.copy(file_path, repaired_path)

        # 3. ZIP olarak aç ve bozuk olabilecek parçaları sil
        with zipfile.ZipFile(repaired_path, 'a') as z:
            bad_files = []
            for f in z.namelist():
                # Genellikle bu meta veri dosyaları bozulur
                if f.startswith('docProps/'):
                    bad_files.append(f)
            
            if bad_files:
                # Yeni bir ZIP oluşturarak bozuk dosyaları dışarıda bırak
                temp_zip_path = repaired_path + ".tmp"
                with zipfile.ZipFile(temp_zip_path, 'w', zipfile.ZIP_DEFLATED) as z_out:
                    for item in z.infolist():
                        if item.filename not in bad_files:
                            buffer = z.read(item.filename)
                            z_out.writestr(item, buffer)
                
                # Geçici onarılmış dosyayı asıl onarılmış dosya ile değiştir
                os.remove(repaired_path)
                os.rename(temp_zip_path, repaired_path)
                logging.info(f"✅ Excel dosyası onarıldı: '{os.path.basename(file_path)}'. Bozuk parçalar: {bad_files}")
                return repaired_path

    except Exception as e:
        logging.error(f"Excel onarımı sırasında hata: {e}")
...
def dosya_oku_akilli(dosya_yolu):
    ...
    try:
        # Güvenlik: with bloğu ile dosyanın her durumda kapanmasını garantile
        try:
            excel_file_obj = pd.ExcelFile(dosya_yolu)
        except zipfile.BadZipFile as e:
            logging.warning(f"Bozuk Excel dosyası tespit edildi ({e}). Otomatik onarım deneniyor...")
            repaired_file_path = _repair_excel_file(dosya_yolu)
            if repaired_file_path:
                excel_file_obj = pd.ExcelFile(repaired_file_path)
            else:
                raise e # Onarım başarısız olursa orijinal hatayı fırlat

        with excel_file_obj as excel:
                    ...
                    if rakamlar:
                        ...
                        if sayfa_no % 2 != 0:
                            ...
                            if sayfa_no != 1:
                                ...
                                # Örn: Sheet1 verisini Sheet2'ye bağla
                                cift_sayfa_adi = sayfa_str.replace(str(sayfa_no), str(sayfa_no + 1))
        ...
        # Excel okuma hatası olduğunda sessiz kalmak yerine logla ve diğer formatları denemeye devam et.
        logging.warning(f"'{os.path.basename(dosya_yolu)}' Excel olarak okunamadı. Diğer formatlar deneniyor... Hata: {excel_err}")
        traceback.print_exc()
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        logging.error(f"'{os.path.basename(dosya_yolu)}' dosyası bilinen hiçbir formatta (Excel, HTML, CSV, RAW) okunamadı. Detaylar: Excel Err={excel_err}, HTML Err={html_e}, CSV Err={csv_e}, RAW Err={agg_e}")
