...
def sutun_adi_normalize_et(col):
    ...
    if c == '924': return 'g924'
    if c == '931': return 'g931'
    if c == '932': return 'g932'
    if c == '960': return 'g960'
    if c == 'bg4': return 'bg4'
    ...
    if c == 'bulut' or 'alçak' in c or 'alcak' in c or c_clean == 'nh' or c_clean.startswith('nh '):
        return 'nh'
...
def header_bul(df):
    ...
    safe_keywords = {
        ...
        'dikine', 'hakim', '1. grup', 'rstçı', 'kap', 'cins', 'bulut', 'rasatçı', 'rasatci', 'rasatlar', 'rasat',
...
def dosya_oku_akilli(dosya_yolu):
    ...
    try:
        ...
        for sayfa in excel.sheet_names:
            ...
            if is_odd_sheet:
                if cift_sayfa_adi:
                    try:
                        ...
                        # --- METAR TEK SAYILI (Ham Rasat) AKILLI ARAMA ---
                        metar_raw_rows = []
                        gmt_col_idx = -1
                        rasat_col_idx = -1
                        rasat_row_start = 0
                        
                        # 1. Başlıklardan (İlk 10 satırdan) Sütun İsimlerini Ara (Örn: 'RASAT' başlığı altındaki C3, C4...)
                        for r_idx in range(min(10, len(df_odd))):
                            for c_idx in range(len(df_odd.columns)):
                                val = str(df_odd.iloc[r_idx, c_idx]).strip().upper().replace(" ", "")
                                if any(k in val for k in ['GMT', 'SAAT', 'ZAMAN', 'TARIH']):
                                    gmt_col_idx = c_idx
                                elif any(k in val for k in ['RASAT', 'BULTEN', 'BÜLTEN', 'MESAJ', 'SIFRE', 'ŞİFRE']):
                                    rasat_col_idx = c_idx
                                    rasat_row_start = r_idx + 1
                            if gmt_col_idx != -1 and rasat_col_idx != -1:
                                break

                        # 2. İçerik Analizi (Eğer başlıklardan bulunamadıysa)
                        if gmt_col_idx == -1 or rasat_col_idx == -1:
                            for c_idx in range(len(df_odd.columns)):
                                sample = df_odd.iloc[:, c_idx].dropna().astype(str)
                                if sample.empty: continue
                                
                                if gmt_col_idx == -1 and (sample.str.match(r'^\d{2}:\d{2}').sum() > 2 or sample.str.match(r'^\d{3,4}Z?$').sum() > 2):
                                    gmt_col_idx = c_idx
                                if rasat_col_idx == -1 and sample.map(len).mean() > 15:
                                    if sample.str.contains(r'KT|MPS|Q\d{4}|CAVOK|=', regex=True).sum() > 2:
                                        rasat_col_idx = c_idx
                        
                        # 3. Fallback (Geleneksel Sütunlar: GMT -> B (1) veya C, RASAT -> C (2) veya D)
                        if gmt_col_idx == -1 and len(df_odd.columns) > 1: gmt_col_idx = 1
                        if rasat_col_idx == -1 and len(df_odd.columns) > 2: rasat_col_idx = 2
                        elif rasat_col_idx == -1 and len(df_odd.columns) > 1: rasat_col_idx = 1
                        ...
                        # Rasatlar (Ham Synop Kodları) DAHA AKILLI ARAMA
                        rasat_baslangic = 4 # Varsayılan D5
                        for r_idx in range(min(20, len(df_odd))):
                            found = False
                            for c_idx in range(min(5, len(df_odd.columns))):
                                val = safe_get(r_idx, c_idx)
                                if val:
                                    val_upper = str(val).strip().upper()
                                    if any(k in val_upper for k in ['RASAT', 'ŞİFRE', 'SIFRE', 'GMT', 'BÜLTEN', 'BULTEN']):
                                        rasat_baslangic = r_idx + 1
                                        found = True
                                        break
                            if found:
                                break
                                
                        for gmt_target in range(24):
                            en_iyi_rasat = None
                            for r in range(max(0, rasat_baslangic - 2), min(rasat_baslangic + 30, len(df_odd))):
                                ilk_hucre = safe_get(r, 0)
                                if ilk_hucre is None: ilk_hucre = safe_get(r, 1) # Bazen B sütununda olabilir
                                
                                if ilk_hucre:
                                    try:
                                        temiz_saat = str(ilk_hucre).strip().upper().replace('Z', '')
                                        
                                        saat_val = None
                                        if ':' in temiz_saat:
                                            saat_val = int(temiz_saat.split(':')[0])
                                        elif len(temiz_saat) >= 3 and temiz_saat.isdigit():
                                            h = int(temiz_saat[:-2])
                                            m = int(temiz_saat[-2:])
                                            if m >= 40: h += 1
                                            if h == 24: h = 0
                                            ...
                                            saat_val = int(float(temiz_saat))
                                            
                                        if saat_val == gmt_target:
                                            # Bu satırda şifreli rasat bul
                                            for c in range(1, len(df_odd.columns)):
                                                val = safe_get(r, c)
                                                if val:
                                                    val_str = str(val).strip()
                                                    val_upper = val_str.upper()
                                                    if len(val_str) > 15 and ('=' in val_str or '333' in val_str or '555' in val_str or '17' in val_str or 'LT' in val_upper):
                                                        en_iyi_rasat = val_str
                                                        break
                                            if en_iyi_rasat:
                                                break
                                    except ValueError:
                            ...
                            if en_iyi_rasat:
                                extracted[gmt_target]['_raw_line'] = en_iyi_rasat
                            else:
                                # Fallback (Eski varsayılan konum - D sütunu)
                                r_varsayilan = rasat_baslangic + (gmt_target // 3)
                                val = safe_get(r_varsayilan, 3)
                                if val and len(str(val)) > 10:
                                    extracted[gmt_target]['_raw_line'] = str(val).strip()
            ...
            if sablon_tipi == 'SINOPTİK':
                    ...
                    logging.info(f"'{sayfa}' sayfası SİNOPTİK dinamik hücre eşleştirmesi ile kaymalara karşı korumalı okundu.")
                    ...
                    logging.info(f"'{sayfa}' sayfası METAR sabit hücre eşleştirmesi ile nokta atışı okundu.")
        ...
        try:
            ...
            logging.error(f"'{os.path.basename(dosya_yolu)}' HTML olarak da okunamadı: {html_e}")
    ...
    # -----------------------------------------------------
    # EKSTRA VERİLERİ (KAPALI RASAT) ANA TABLOYA AKTAR VE GMT NORMALİZASYONU
    # -----------------------------------------------------
    ...
    if 'gmt' in df_all.columns:
        gmt_num = df_all['gmt'].apply(safe_gmt_extract)
        gmt_exact = df_all['gmt'].apply(exact_gmt_extract)
        
        df_all['gmt_exact'] = gmt_exact
        df_all['gmt'] = gmt_num
    else:
        gmt_num = pd.Series([float('nan')] * len(df_all))
        gmt_exact = pd.Series([""] * len(df_all))
