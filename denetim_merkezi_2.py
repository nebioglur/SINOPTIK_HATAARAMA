...
def _get_scalar_val(value):
    """
    Gelen değerin bir dizi (array/list/Series) olması durumunda ilk elemanını,
    değilse kendisini döndürerek skaler bir değer garante eder.
    Bu, 'ValueError: The truth value of an empty array is ambiguous' hatasını önler.
    """
    if hasattr(value, '__iter__') and not isinstance(value, (str, bytes)):
        try:
            # Pandas Serisi veya NumPy dizisi ise ilk elemanı al
            return value.iloc[0] if hasattr(value, 'iloc') and not value.empty else (value[0] if len(value) > 0 else None)
...
def _to_float(val):
    ...
    if hasattr(val, '__iter__') and not isinstance(val, (str, bytes)):
        val = _get_scalar_val(val)
    try: return float(val) if pd.notna(val) else None
    except (ValueError, TypeError): return None

def _check_pressure(data, hata_dict, get_sinoptik_prev=None):
    """Basınç (p, p0) ve tandans (a, ppp) gruplarını kontrol eder."""
    kodlar, aciklamalar = [], []
    ...
    if a == 9: kodlar.append("h61"); aciklamalar.append(hata_dict.get("h61"))
    # h63: Tandans karakteri 4 iken değişim 0 olur
    if a == 4 and pd.notna(ppp) and ppp != 0: kodlar.append("h63"); aciklamalar.append(hata_dict.get("h63"))
    # h62: a!=0,4,5 iken ppp=0
    if pd.notna(a) and a not in [0, 4, 5] and pd.notna(ppp) and ppp == 0:
        kodlar.append("h62"); aciklamalar.append(hata_dict.get("h62"))
    # h64: ppp çok fazla
    if pd.notna(ppp) and ppp > 20: kodlar.append("h64"); aciklamalar.append(hata_dict.get("h64"))

    # h65 & h66: Geçmiş 3 saatlik basınç ile tandans uyumu
    if get_sinoptik_prev:
        prev_data = get_sinoptik_prev(3)
        if prev_data is not None and not prev_data.empty:
            p_prev = prev_data["p_sin"].values[0] if "p_sin" in prev_data.columns else None
            if pd.isna(p_prev): p_prev = prev_data["p"].values[0] if "p" in prev_data.columns else None
            if pd.isna(p_prev): p_prev = prev_data["p0_sin"].values[0] if "p0_sin" in prev_data.columns else None
            if pd.isna(p_prev): p_prev = prev_data["p0"].values[0] if "p0" in prev_data.columns else None
            
            p_curr = p
            if pd.isna(p_curr): p_curr = p0
            ...
            if pd.notna(p_curr) and pd.notna(p_prev) and pd.notna(a) and pd.notna(ppp):
                try:
                    ...
                    prev_sin_msg = "Yok"
                    if "bulten_sin" in prev_data.columns:
                        prev_sin_msg = str(prev_data["bulten_sin"].values[0]).strip()
                    elif "bulten" in prev_data.columns:
                        prev_sin_msg = str(prev_data["bulten"].values[0]).strip()
                    if prev_sin_msg.lower() in ["nan", "none", ""]: prev_sin_msg = "Yok"
                    
                    p_curr_str = f"{p_curr:.1f}"
                    p_prev_str = f"{p_prev:.1f}"
                    diff_str = f"{diff:.1f}"
                    ek_bilgi = f" (Mevcut P: {p_curr_str}, Önceki P: {p_prev_str}, Fark: {diff_str}) 💡 İLGİLİ GEÇMİŞ SİNOPTİK: {prev_sin_msg}"
                    
                    # h66: Tandans değişimi tandans karakteri ile uyumsuz
                    if a in [0, 1, 2, 3] and diff < -0.1:
                        kodlar.append("h66"); aciklamalar.append(str(hata_dict.get("h66", "")) + ek_bilgi)
                    elif a in [5, 6, 7, 8] and diff > 0.1:
                        kodlar.append("h66"); aciklamalar.append(str(hata_dict.get("h66", "")) + ek_bilgi)
                    
                    # h65: Tandans değişimi 3 saat önceki basınçla uyumsuz
                    if abs(diff_abs - float(ppp)) >= 0.3:
                        kodlar.append("h65"); aciklamalar.append(str(hata_dict.get("h65", "")) + ek_bilgi)
                except:
                    pass

    return kodlar, aciklamalar


def _check_precipitation_and_indicators(data, hata_dict, get_metar_history, get_sinoptik_prev, df_metar, clean_trend):
    """Yağış (6RRRtR) ve indikatör (ir, ix) gruplarını kontrol eder."""
    kodlar, aciklamalar = [], []
    ir, rrr, ix, w1, w2, ww, tr, gmt, hadise_kayit, rrr_toplam = (data.get(k) for k in ['ir', 'rrr', 'ix', 'w1', 'w2', 'ww', 'tr', 'gmt', 'hadise_kayit', 'rrr_toplam'])
...
def _check_wind(data, hata_dict):
    """Rüzgar (dd, ff, 910, 911) gruplarını kontrol eder."""
    kodlar, aciklamalar = [], []
    dd, ff, g910_val, g911_val = (data.get(k) for k in ['dd', 'ff', 'g910', 'g911'])
...
def _check_temperature_and_ground(data, hata_dict, get_sinoptik_prev):
    """Sıcaklık (T, Td, Tx, Tn) ve Toprak (Tg, E) gruplarını kontrol eder."""
    kodlar, aciklamalar = [], []
    t, td, tx, tn, tg, e, gmt, rasatlar = (data.get(k) for k in ['t', 'td', 'tx', 'tn', 'tg', 'e', 'gmt', 'rasatlar'])
...
def _check_clouds(data, hata_dict, df_metar, s, clean_trend):
    """Bulut (N, Nh, h, CL, CM, CH) gruplarını kontrol eder."""
    kodlar, aciklamalar = [], []
    n, nh, cl, cm, ch, h, ww, hadise_kayit = (data.get(k) for k in ['n', 'nh', 'cl', 'cm', 'ch', 'h', 'ww', 'hadise_kayit'])
    gmt = data.get('gmt')
    ...
    if pd.notna(n) and n == 9:
        ...
        if pd.notna(h):
            ...
            elif pd.notna(data.get('vv')) and data.get('vv') < 10 and h >= 3:
        ...
        gorus_dusuk_mu = pd.notna(data.get('vv')) and data.get('vv') < 50
        ...
        if not gorus_dusuk_mu and pd.notna(data.get('vv')):
            kodlar.append("h312")
            aciklamalar.append(f"Gökyüzü görünmüyor (N=9) olarak kodlanmış ancak yatay görüş mesafesi oldukça yüksek (VV={int(data.get('vv'))}).")
...
def get_h_from_metar(metar_str):
    if not isinstance(metar_str, str): return None
    import re
    matches = re.findall(r'\b(FEW|SCT|BKN|OVC|VV)(\d{3})\b', metar_str)
    if not matches:
        if 'SKC' in metar_str or 'CAVOK' in metar_str or 'NSC' in metar_str:
            return 9
        return None
    
    # 1. Sadece alçak seviye bulutlarını (<= 6500 ft / 065) filtrele
    low_clouds = [m for m in matches if int(m[1]) <= 65]
    if not low_clouds:
        # Alçak bulut yoksa en alçak orta/yüksek bulutu al
        low_clouds = matches
        
    coverage_rank = {'VV': 4, 'OVC': 4, 'BKN': 3, 'SCT': 2, 'FEW': 1}
    
    # Kapanlılık miktarına göre (Büyükten küçüğe), eşitse yüksekliğe göre (Küçükten büyüğe) sırala
    low_clouds.sort(key=lambda m: (-coverage_rank.get(m[0], 0), int(m[1])))
    
    target_ft = int(low_clouds[0][1]) * 100
    if target_ft < 164: return 0
    elif target_ft <= 328: return 1
    elif target_ft <= 656: return 2
    elif target_ft <= 984: return 3
    elif target_ft <= 1968: return 4
    elif target_ft <= 3280: return 5
    elif target_ft <= 4920: return 6
    elif target_ft <= 6560: return 7
    elif target_ft <= 8200: return 8
    else: return 9

def hata_analizi_yap(birlesik_df, df_metar, progress_callback=None, df_sinoptik=None):
    """
    ...
    def get_arsiv_df(dt_target):
        ...
        # DİNAMİK YOL: Arşiv klasörünü kullanıcının masaüstündeki check klasöründe ara.
        check_klasoru = os.path.join(os.path.expanduser("~"), "Desktop", "check")
        arsiv_yolu = os.path.join(check_klasoru, "Arsiv", f"{y}_{m:02d}")
        hedef_dosya = None
    ...
    def hata_ara(s):
        ...
        # --- KULLANICI İSTEĞİ: EXCEL'deki Bg1, Bg2, Bg3, Bg4 sütunları 8NsChshs formatındadır.
        # Bu gruplar, ana 8NhCLCMCH grubuyla çelişmez, ek katman bilgisidir.
        # Bu nedenle, bu gruplar varsa CL/CM/CH karşılaştırmasını (h307 vb.) atla.
        excel_ek_bulut_grubu_var = False
        for bg_col in ["bg1", "bg2", "bg3", "bg4"]:
            bg_val = _get_val(s, bg_col)
            if pd.notna(bg_val):
                try:
                    # Değerin 8 ile başlayan 5 haneli bir sayı olup olmadığını kontrol et
                    bg_str = str(int(float(bg_val)))
                    if len(bg_str) == 5 and bg_str.startswith('8'):
                        excel_ek_bulut_grubu_var = True
                        break
                except (ValueError, TypeError):
                    # Değer sayısal değilse veya format uymuyorsa devam et
                    pass
        # --- BİTTİ ---
        ...
        # Tüm verileri bir sözlükte topla
        data = {
            'ir': ir, 'rrr': rrr, 'ix': ix, 'w1': w1, 'w2': w2, 'tr': tr, 't': t, 'td': td,
            'tx': tx, 'tn': tn, 'tg': tg, 'n': n, 'nh': nh, 'cl': cl, 'cm': cm, 'ch': ch,
            'dd': dd, 'ff': ff, 'vv': vv, 'a': a, 'ppp': ppp, 'p': p, 'p0': p0, 'e': e,
            'h': h, 'ww': ww, 'g924': g924, 'g931': g931, 'g932': g932, 'g960': g960,
            'hadise_kayit': hadise_kayit, 'rasatlar': rasatlar, 'gunes': gunes, 'buhar': buhar,
            'rh': rh, 'gmt': gmt, 'sayfa': s.get("sayfa"), 'bulten_metar': s.get("bulten_metar"),
            'bulten': s.get("bulten"), 'vv_metar': s.get("vv_metar"), 'istasyon_no': s.get('istasyon_no_sin'),
            'excel_ek_bulut_grubu_var': excel_ek_bulut_grubu_var, 'g910': s.get("g910"),
            'g911': s.get("g911"), 'rrr_toplam': _get_val(s, "rrr_toplam")
        }
        ...
        if pd.notna(rasatlar) and ":" not in str(rasatlar) and len(str(rasatlar).strip()) >= 5 and SynopDecoder is not None:
            try:
                ...
                if s_data_tmp:
                    def is_empty(key): return pd.isna(data.get(key)) or str(data.get(key)).strip() == ""
                    if is_empty('w1') and 'gecmis_hava1' in s_data_tmp: data['w1'] = w1 = _to_float(s_data_tmp['gecmis_hava1'])
                    if is_empty('w2') and 'gecmis_hava2' in s_data_tmp: data['w2'] = w2 = _to_float(s_data_tmp['gecmis_hava2'])
                    if is_empty('ww') and 'halihazir_hava' in s_data_tmp: data['ww'] = ww = _to_float(s_data_tmp['halihazir_hava'])
                    if is_empty('tr') and 'yagis_suresi_kod' in s_data_tmp: data['tr'] = tr = _to_float(s_data_tmp['yagis_suresi_kod'])
                    if is_empty('rrr') and 'yagis_miktari_kod' in s_data_tmp: data['rrr'] = rrr = _to_float(s_data_tmp['yagis_miktari_kod'])
                    if is_empty('ir') and 'ir' in s_data_tmp: data['ir'] = ir = _to_float(s_data_tmp['ir'])
                    if is_empty('ix') and 'ix' in s_data_tmp: data['ix'] = ix = _to_float(s_data_tmp['ix'])
        ...
        def get_dt_suan():
            t_str = s.get("sayfa")
            if pd.isna(t_str) or str(t_str).strip() == "" or str(t_str).strip().lower() == "nan":
                t_str = s.get("Tarih")
            # DEBUG
            if not getattr(get_dt_suan, 'has_printed', False):
                print(f"DEBUG get_dt_suan: t_str={t_str}, gmt={gmt}")
                get_dt_suan.has_printed = True
        ...
        def get_metar_history(hours_back):
            ...
            start_time = dt_s - datetime.timedelta(hours=hours_back)
            end_time = dt_s
            
            tarihler = [(start_time + datetime.timedelta(hours=i)).strftime('%d.%m.%Y') for i in range(hours_back+1)]
            sonuc = df_metar[df_metar["sayfa"].isin(tarihler)]
            ...
            for _, r in sonuc.iterrows():
                ...
                gmt_val = r.get("gmt")
                if pd.isna(sayfa): continue
                
                try:
                    g, a, y = map(int, str(sayfa).split('.'))
                    rounded_date = datetime.datetime(y, a, g)
                    
                    if pd.notna(gmt_exact) and str(gmt_exact).strip() != "":
                    ...
                    elif pd.notna(gmt_val):
                        actual_dt = rounded_date.replace(hour=int(float(gmt_val)))
                    else:
                        continue
                except:
                    continue
                
                # 1. Normal periyot iindeki METAR'lar al (dt_b <= actual_dt <= dt_s)
                if dt_b <= actual_dt <= dt_s:
                    r_cleared = clear_boundary_re(r)
                    valid_rows.append(r_cleared)
        ...
        def get_sinoptik_prev(hours_back):
            ...
            # GMT sütunu Excel'de 2100, 0000 gibi 4 haneli veya 21, 0 gibi 2 haneli olabilir.
            # dt_prev.hour = 0..23 (örn: 21). Buna karşılık gelen 4 haneli değer: 2100, 0000 gibi.
            gmt_hour = dt_prev.hour          # ör: 21
            gmt_4digit = gmt_hour * 100      # ör: 2100

            def gmt_esles(series):
                """Hem 21 hem 2100 formatını kabul eder."""
                try:
                    s = series.astype(str).str.replace('.0', '', regex=False).str.zfill(2)
                    return (s == str(gmt_hour).zfill(2)) | (s == str(gmt_4digit).zfill(4))
                except:
                    return series == gmt_hour

            def tarih_esles(series):
                """Tarih sütununu datetime'a çevirerek format ve padding hatalarını (24.7 vs 24.07) çözer."""
                try:
                    s_dt = pd.to_datetime(series, errors='coerce', dayfirst=True)
                    return s_dt.dt.date == dt_prev.date()
                except:
                    s = series.astype(str).str.strip()
                    p_str_unpadded = f"{dt_prev.day}.{dt_prev.month}.{dt_prev.year}"
                    return (s == p_str) | (s == p_str_unpadded)

            if df_sinoptik is not None and "Tarih" in df_sinoptik.columns:
                gmt_col = "gmt" if "gmt" in df_sinoptik.columns else ("GMT" if "GMT" in df_sinoptik.columns else None)
                if gmt_col:
                    sonuc = df_sinoptik[tarih_esles(df_sinoptik["Tarih"]) & gmt_esles(df_sinoptik[gmt_col])]
                else:
                    sonuc = pd.DataFrame()
            elif "Tarih" in birlesik_df.columns:
                gmt_col = "gmt" if "gmt" in birlesik_df.columns else ("GMT" if "GMT" in birlesik_df.columns else None)
                if gmt_col:
                    sonuc = birlesik_df[tarih_esles(birlesik_df["Tarih"]) & gmt_esles(birlesik_df[gmt_col])]
                else:
                    sonuc = pd.DataFrame()
            else:
                sonuc = pd.DataFrame()

            if sonuc.empty and dt_s.date() != dt_prev.date():
                df_arsiv = get_arsiv_df(dt_prev)
                if not df_arsiv.empty and "Tarih" in df_arsiv.columns:
                    gmt_col = "gmt" if "gmt" in df_arsiv.columns else ("GMT" if "GMT" in df_arsiv.columns else None)
                    if gmt_col:
                        sonuc = df_arsiv[tarih_esles(df_arsiv["Tarih"]) & gmt_esles(df_arsiv[gmt_col])]
            return sonuc
        ...
        def is_oraj(w):
            try: return int(w) in [17, 29] or (95 <= int(w) <= 99)
                ...
                cw = clean_trend(w)
                # VCTS, istasyon sahası dışındaki civar gök gürültülü sağanağı ifade eder.
                # Bu durumda sinoptik geçmiş hava W1/W2 için Oraj (9) beklenmez.
                if "VCTS" in cw:
                    return False
        ...
        # --- MODÜLER KONTROLLER ---
        new_k, new_a = _check_precipitation_and_indicators(data, hata_dict, get_metar_history, get_sinoptik_prev, df_metar, clean_trend)
        kodlar.extend(new_k); aciklamalar.extend(new_a)

        new_k, new_a = _check_wind(data, hata_dict)
        kodlar.extend(new_k); aciklamalar.extend(new_a)

        new_k, new_a = _check_temperature_and_ground(data, hata_dict, get_sinoptik_prev)
        kodlar.extend(new_k); aciklamalar.extend(new_a)

        new_k, new_a = _check_clouds(data, hata_dict, df_metar, s, clean_trend)
        kodlar.extend(new_k); aciklamalar.extend(new_a)
        ...
        new_k, new_a = _check_pressure(data, hata_dict, get_sinoptik_prev)
        kodlar.extend(new_k); aciklamalar.extend(new_a)
        ...
        if pd.notna(ww) and ww in [1, 2, 3] and pd.notna(n):
            ...
            if dt_suan:
                for h_back in [1, 2, 3]:
                    ...
                    if p_row.empty and dt_suan.date() != dt_prev.date():
                        df_arsiv = get_arsiv_df(dt_prev)
                    ...
                    if not p_row.empty:
                        with open('p_row_DEBUG.txt', 'a', encoding='utf-8') as fdbg:
                            fdbg.write(f"--- P_ROW DUMP ---\n")
                            for idx, r in p_row.iterrows():
                                fdbg.write(f"Row {idx}:\n")
                                for col in p_row.columns:
                                    fdbg.write(f"  {col}: {r[col]}\n")
                            fdbg.write(f"------------------\n")

                        for n_key in ["n_sin", "n", "n_metar", "T. Kp."]:
                        ...
                        for h_key in ["h_sin", "h", "h_metar", "alcak_bulut_yuksekligi"]:
                        ...
                        if prev_h is None:
                            # METAR'dan fallback
                            for b_key in ["METAR - Şifreli Mesaj", "bulten_metar", "bulten", "RASATLAR"]:
                                if b_key in p_row.columns:
                                    for b_val in p_row[b_key].values:
                                        if pd.notna(b_val) and isinstance(b_val, str):
                                            hm = get_h_from_metar(b_val)
                                            if hm is not None:
                                                prev_h = float(hm)
                                                break
                                    if prev_h is not None: break
                ...
                if prev_n is not None:
                    if pd.isna(h):
                        # Güncel h yoksa METAR'dan fallback
                        for b_key in ["METAR - Şifreli Mesaj", "bulten_metar", "bulten", "RASATLAR"]:
                            if b_key in s and pd.notna(s[b_key]) and isinstance(s[b_key], str):
                                hm = get_h_from_metar(s[b_key])
                                if hm is not None:
                                    h = float(hm)
                                    break
                    ...
                    if n < prev_n: expected_ww = 1
                        ...
                        if 'prev_h' in locals() and prev_h is not None and pd.notna(h):
                            try:
                                ...
                                # Eğer rasatçı ww=02 (değişmedi) girdiyse ve h farkı sadece 1 ise
                                # (Örn: 3000ft sınırı nedeniyle h=5 ve h=6 arasında gidip geliyorsa) tolere et.
                                if ww == 2 and abs(h_val - prev_h) == 1:
                                    expected_ww = 2
                            except: pass
                    h_kiyas_yapilabildi = ('prev_h' in locals() and prev_h is not None and pd.notna(h))
                    ...
                    if hata_var:
                        ...
                        # SİNOPTİK GEÇMİŞİNİ MESAJIN SONUNA EKLİYORUZ
                        try:
...
#             aciklamalar.append(hata_dict.get("h197"))
        ...
        try:
            if pd.notna(gmt):
                ...
                if metar_kesit is None or metar_kesit.empty:
                    # Geçmiş METAR verisi yoksa karşılaştırma yapmayı atla (sahte hataları önler)
                    raise ValueError("METAR geçmiş verisi boş")
                ww_cols = [c for c in ["ww", "ww2", "ww3"] if c in df_metar.columns]
                bulten_col = "bulten" if "bulten" in df_metar.columns else None
                ...
                if pd.notna(g931):
                    try:
                        ...
                        if ss <= 55: ss_cm = float(ss)
                        elif 56 <= ss <= 90: ss_cm = float((ss - 50) * 10)
                        elif 91 <= ss <= 96: ss_cm = float((ss - 90) * 0.1)
                        elif ss == 97: ss_cm = 0.0
                        elif ss == 98: ss_cm = 401.0
                        else: ss_cm = float(ss)
                    except: pass
        ...
        if pd.notna(rasatlar) and not excel_ek_bulut_grubu_var:
            try:
                rasat_str = str(rasatlar)
                
                # Ana grup: 886// formatını ara
                m_886 = re.search(r'\b8(\d)(\d)(//)?\b', rasat_str)
                
                # Bölüm 3: 333 sonrası ek bulut katmanlarını (88...) ara
                m_333 = re.search(r'\b333\b', rasat_str)
                has_section3_clouds = False
                
                if m_333:
                    bolum_3 = rasat_str[m_333.end():]
                    # 555 bulursa kesme
                    if "555" in bolum_3:
                        bolum_3 = bolum_3.split("555")[0]
                    
                    # 8NsChshs formatındaki bulut katmanlarını ara (Örn: 88702, 87630)
                    section3_clouds = re.findall(r'\b8\d{4}\b', bolum_3)
                    has_section3_clouds = len(section3_clouds) > 0
                
                # Tutarlılık Kontrol Mantığı:
                # Ana grup varsa ama Bölüm 3'te ek bulut grubu yoksa, veya
                # Her ikisi de varsa ve tutarlı değilse h307 verilir.
                # ANCAK Bölüm 3'te ek bulut grupları varsa (Excel'den girilen), kontrol atlanır.
                
                if m_886 and not has_section3_clouds:
                    # Ana grup var ama Bölüm 3 detayı yok
                    # WMO Kuralı: Eğer ana grupta belirli bir bulut tasvir edilmişse,
                    # Bölüm 3'te de o bulutun detayları olmalı.
                    # Ancak bazı durumlarda (Example: 886// sadece boş gökyüzü gösteriyor)
                    # Bölüm 3 detayı isteğe bağlı olabilir.
                    
                    nh_val = int(m_886.group(1))  # Alçak/Orta Kapalılığı (0-8)
                    cl_val = int(m_886.group(2)) if m_886.group(2).isdigit() else 0  # Alçak Bulut
                    
                    # Eğer nh=0 (Boş) ve cl=0 (Bulut yok) ise Bölüm 3 detayı gerekmez
                    # Ama nh > 0 ise (Bulut varsa) Bölüm 3 detayı beklenir
                    if nh_val > 0:
                        # Bölüm 3'te detay bulunmadı - DİKKAT uyarısı
                        if "h307" not in kodlar:
                            kodlar.append("h307")
                            aciklamalar.append("Ana SİNOPTİK bulut grubu (886//) gökyüzünde bulut gösterirken (Nh>0), Bölüm 3'teki detaylı bulut katmanı grubu (88...) eksik. İşletme kuralınıza göre Bölüm 3 detayı gerekebilir.")
                decoder = SynopDecoder()
                s_data = decoder.decode_line(rasat_str)
                if s_data:
                    # Şifreden gelen bulut verileri
                    raw_nh = s_data.get('alcak_bulut_miktari')
                    raw_cl = s_data.get('alçak_bulut_cinsi')
                    raw_cm = s_data.get('orta_bulut_cinsi')
                    raw_ch = s_data.get('yüksek_bulut_cinsi')

                    # Excel'den gelen bulut verileri (fonksiyonun başındaki değişkenler)
                    excel_nh = nh
                    excel_cl = cl
                    excel_cm = cm
                    excel_ch = ch

                    # Eğer Excel'deki değerlerden herhangi birine yanlışlıkla tam 8 grubu (ör: 86250) girildiyse ayıkla:
                    ...
                    group_8 = None
                    for val in [excel_nh, excel_cl, excel_cm, excel_ch]:
                        group_8 = extract_8_group(val)
                        if group_8: break

                    if group_8:
                        excel_nh = group_8[1] if group_8[1] != '/' else None
                        excel_cl = group_8[2] if group_8[2] != '/' else None
                        excel_cm = group_8[3] if group_8[3] != '/' else None
                        excel_ch = group_8[4] if group_8[4] != '/' else None

                    def compare_and_log(raw_val, excel_val, col_name, code):
                        if pd.notna(raw_val) and pd.notna(excel_val):
                            try:
                                if int(raw_val) != int(excel_val):
                                    if code not in kodlar:
                                        kodlar.append(code)
                                        aciklamalar.append(f"Şifreli SİNOPTİK mesajı ile Excel verisi çelişiyor! Şifreye göre {col_name}={int(raw_val)}, Excel'de {col_name}={int(excel_val)}.")
                            except (ValueError, TypeError):
                                pass

                    compare_and_log(raw_nh, excel_nh, "Nh", "h307")
                    compare_and_log(raw_cl, excel_cl, "CL", "h307")
                    compare_and_log(raw_cm, excel_cm, "CM", "h307")
                    compare_and_log(raw_ch, excel_ch, "CH", "h307")

                    # Ek olarak, Bölüm 3'teki (333) bulut katmanlarını da kontrol et
                    if 'cloud_layers' in s_data:
                        for layer in s_data['cloud_layers']:
                            # Bu katmanlar genellikle Excel'deki Bg1, Bg2, Bg3, Bg4 sütunlarına karşılık gelir.
                            # Şimdilik sadece şifre içi tutarlılık kontrolü yapılıyor.
                            # İleride bu katmanlar da Excel ile karşılaştırılabilir.
                            pass
        ...
        # h307 kodunu denetlemeyi bırak (kullanıcı isteği)
        if "h307" in kodlar:
            new_kodlar = []
            new_aciklamalar = []
            for k, a in zip(kodlar, aciklamalar):
                if k != "h307":
                    new_kodlar.append(k)
                    new_aciklamalar.append(a)
            kodlar = new_kodlar
            aciklamalar = new_aciklamalar
        ...
        if kodlar:
            ...
            try:
                if get_sinoptik_prev:
                    s_list = []
                    for h_back in [3, 6]:
                        prev_data = get_sinoptik_prev(h_back)
                        if prev_data is not None and not prev_data.empty:
                            b_col_s = "bulten_sin" if "bulten_sin" in prev_data.columns else "bulten"
                            if b_col_s in prev_data.columns:
                                p_raw = str(prev_data[b_col_s].values[0]).strip()
                                if p_raw and p_raw.lower() not in ['nan', 'none', '<na>', '-', '']:
                                    p_gmt = int(float(prev_data["gmt"].values[0])) if "gmt" in prev_data.columns else 0
                                    s_list.append(f"[{p_gmt:02d}00Z] {p_raw}")
                    if s_list:
                        ek_sinoptik_bilgisi = "İLGİLİ SİNOPTİK GEÇMİŞİ:\n" + "\n".join(s_list)
            except:
                pass
                
            return ("Hata Var", ", ".join(kodlar), " | ".join(aciklamalar), ek_metar_bilgisi, ek_sinoptik_bilgisi)
        else:
            return ("Hata Yok", "", "", "", "")

    kayitlar = birlesik_df.to_dict('records')
    sonuclar = [None] * len(kayitlar)
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        future_to_index = {executor.submit(hata_ara, r): idx for idx, r in enumerate(kayitlar)}
        for future in concurrent.futures.as_completed(future_to_index):
            idx = future_to_index[future]
            try:
                sonuclar[idx] = future.result()
            except Exception as e:
                sonuclar[idx] = ("Hata Var", "VAL_CRASH", f"Hata Analiz Motoru Hatası: {e}", "", "")
            ...
            if progress_callback:
                ...
                if completed > 1:
                    ...
                    remaining = 0.0
                try:
                    should_continue = progress_callback(completed, total, pct, remaining)
    ...
    sonuc = pd.DataFrame(sonuclar, columns=["ANALİZ_SONUCU", "HATA_KODLARI", "HATA_ACIKLAMALARI", "EK_METAR_BILGISI", "EK_SINOPTIK_BILGISI"], index=birlesik_df.index)
    ...
    if "EK_METAR_BILGISI" in birlesik_df.columns:
        ...
        if b_col in birlesik_df.columns:
            ...
            for idx in birlesik_df[mask].index:
                ...
                if mevcut:
                    if ek not in mevcut: birlesik_df.at[idx, b_col] = mevcut + "\n\n" + ek
                else: birlesik_df.at[idx, b_col] = ek
        birlesik_df.drop(columns=["EK_METAR_BILGISI"], inplace=True)
        
    if "EK_SINOPTIK_BILGISI" in birlesik_df.columns:
        b_col_s = "bulten_sin" if "bulten_sin" in birlesik_df.columns else "bulten"
        if b_col_s in birlesik_df.columns:
            mask = birlesik_df["EK_SINOPTIK_BILGISI"].astype(str).str.strip() != ""
            for idx in birlesik_df[mask].index:
                mevcut = _clean_bulten(birlesik_df.at[idx, b_col_s])
                ek = str(birlesik_df.at[idx, "EK_SINOPTIK_BILGISI"]).strip()
                if mevcut:
                    if ek not in mevcut: birlesik_df.at[idx, b_col_s] = mevcut + "\n\n" + ek
                else: birlesik_df.at[idx, b_col_s] = ek
        birlesik_df.drop(columns=["EK_SINOPTIK_BILGISI"], inplace=True)
