...
class SynopDecoder:
    ...
    def decode_line(self, line):
        ...
        self.errors = [] # Her yeni satır çözümlemesinde hataları sıfırla
            ...
            if mode == "section1":
                ...
                try:
                    if indicator == '1' and '/' not in grp:
                        ...
                        data["kar_kalinligi"] = int(grp[3:])
                        data['raw_groups']['kar_kalinligi'] = grp
                ...
                if len(tok) == 5:
                    ...
                    if ind == '1':
                        ...
                        if tok.startswith('910'):
                            ...
                            res.append(f"{tok} : Taze Kar Kalınlığı (9 31 ss)")
                            res.append(f"  931: Taze kar kalınlığı indikatörü.")
                            res.append(f"  {tok[3:5]}: Taze kar kalınlığı {int(tok[3:5]) if tok[3:5].isdigit() else tok[3:5]} cm.")
                        elif tok.startswith('932'):
                            res.append(f"{tok} : Maksimum Kar Kalınlığı (9 32 ss)")
                            res.append(f"  932: Maksimum kar kalınlığı indikatörü.")
                            res.append(f"  {tok[3:5]}: Maksimum kar kalınlığı {int(tok[3:5]) if tok[3:5].isdigit() else tok[3:5]} cm.")
