...
def get_metar_h_code(bulten_str):
    if not isinstance(bulten_str, str) or not bulten_str.strip():
        return None
    # 1. Sky obscured (VV)
    vv_match = re.search(r'\bVV(///|\d{3})', bulten_str)
    if vv_match:
        return '/'  # "Gökyüzü görülemiyor ise; h = / olarak rapor edilir"
    
    # 2. Extract lowest cloud height
    cloud_matches = re.findall(r'\b(?:FEW|SCT|BKN|OVC)(\d{3})', bulten_str)
    if not cloud_matches:
        # "Gökyüzünde bulut yoksa h=9 kodlanır" -> NSC, CAVOK, SKC, vs.
        if re.search(r'\b(CAVOK|NSC|SKC|NCD)\b', bulten_str):
            return 9
        return None
    
    # En alçak tabana sahip bulut:
    try:
        lowest_ft = min([int(h) * 100 for h in cloud_matches])
        if lowest_ft <= 164: return 0
        elif lowest_ft <= 328: return 1
        elif lowest_ft <= 656: return 2
        elif lowest_ft <= 984: return 3
        elif lowest_ft <= 1968: return 4
        elif lowest_ft <= 3280: return 5
        elif lowest_ft <= 4920: return 6
        elif lowest_ft <= 6560: return 7
        elif lowest_ft <= 8200: return 8
        else: return 9
    except: return None
...
def _check_pressure(data, hata_dict):
...
def hata_analizi_yap(birlesik_df, df_metar, progress_callback=None):
    """
    ...
    def hata_ara(s):
        ...
        def get_metar_history(hours_back):
            ...
            dt_b = dt_s - datetime.timedelta(hours=hours_back, minutes=10)
            ...
            ay_degisti = dt_s.date() != dt_b.date()
            candidates = df_metar
            if ay_degisti:
                df_arsiv = get_arsiv_df(dt_b)
                if not df_arsiv.empty:
                    candidates = pd.concat([df_arsiv, df_metar], ignore_index=True)
        ...
        # h378: Geçmiş 1 Saatlik METAR Gelişim Kontrolü
        if pd.notna(ww) and ww in [1, 2, 3] and pd.notna(n):
            prev_n = None
            prev_h = None
            dt_suan = get_dt_suan()
            m_row_1hr = pd.DataFrame()
            if dt_suan:
                # SİNOPTİK saatinin 1 saat öncesini bul (18:00 SİNOPTİK -> 17:00 -> 16:50 METAR)
                dt_prev_metar = dt_suan - datetime.timedelta(hours=1)
                prev_sayfa_m = dt_prev_metar.strftime('%d.%m.%Y')
                prev_gmt_m = float(dt_prev_metar.hour)
                
                m_row_1hr = birlesik_df[(birlesik_df["sayfa"] == prev_sayfa_m) & (birlesik_df["gmt"].apply(get_m_hour) == prev_gmt_m)]
                if "mesaj_tipi" in m_row_1hr.columns:
                    m_row_1hr = m_row_1hr[m_row_1hr["mesaj_tipi"].notna()]
                    
                if not m_row_1hr.empty:
                    # 1 saat önceki METAR'ın N değerini çek
                    n_col = "n_metar" if "n_metar" in m_row_1hr.columns else "n"
                    for val in m_row_1hr[n_col].values:
                    ...
                    # 1 saat önceki METAR'ın h değerini (Bülten metninden) hesapla
                    bulten_col = "bulten_metar" if "bulten_metar" in m_row_1hr.columns else "bulten"
                    for bulten_val in m_row_1hr[bulten_col].values:
                        if pd.notna(bulten_val):
                            prev_h_code = get_metar_h_code(str(bulten_val))
                            if prev_h_code is not None and prev_h_code != '/':
                                try: prev_h = float(prev_h_code)
                                except: pass
                                break
                ...
                if prev_n is not None:
                    ...
                    if hata_var:
                        ...
                        if expected_ww == 1:
                            aciklamalar.append(f'ww={int(ww):02d} kodlanmış ancak 1 saat önceki METAR rasatına göre bulutluluk azalmış veya yüksekliği artmış (Önceki N={int(prev_n)}, Güncel N={int(n)}{h_str}). [TAVSIYE_WW=01]')
                        elif expected_ww == 2:
                            aciklamalar.append(f'ww={int(ww):02d} kodlanmış ancak 1 saat önceki METAR rasatına göre bulutluluk değişmemiş (Önceki N={int(prev_n)}, Güncel N={int(n)}{h_str}). [TAVSIYE_WW=02]')
                        elif expected_ww == 3:
                            aciklamalar.append(f'ww={int(ww):02d} kodlanmış ancak 1 saat önceki METAR rasatına göre bulutluluk artmış veya yüksekliği azalmış (Önceki N={int(prev_n)}, Güncel N={int(n)}{h_str}). [TAVSIYE_WW=03]')

                        # 1 SAAT ÖNCEKİ METAR GEÇMİŞİNİ EKLİYORUZ
                        try:
                            raw_col_m = '_raw_line_metar' if '_raw_line_metar' in m_row_1hr.columns else '_raw_line'
                            if raw_col_m in m_row_1hr.columns:
                                for raw_val in m_row_1hr[raw_col_m].values:
                                    if pd.notna(raw_val) and str(raw_val).strip() != '':
                                        aciklamalar[-1] += f"\n\n👉 1 SAAT ÖNCEKİ REFERANS METAR ({prev_gmt_m}Z):\n{str(raw_val).strip()}"
                                        break
                        except: pass

                    # Eşzamanlı N Çapraz Kontrolü (Örn: 15:00 SİNOPTİK N ile 14:50 METAR N aynı mı?)
                    if pd.notna(n):
                        n_metar_val = None
                        if 'n_metar' in s and pd.notna(s['n_metar']):
                            try: n_metar_val = float(s['n_metar'])
                            except: pass
                            
                        n_sin_val = None
                        try: n_sin_val = float(n)
                        except: pass
                        
                        if n_metar_val is not None and n_sin_val is not None:
                            if n_sin_val != n_metar_val:
                                kodlar.append('h378_N_UYUMSUZ')
                                aciklamalar.append(f'Aynı saatteki SİNOPTİK bulutluluğu (N={int(n_sin_val)}) ile METAR bulutluluğu (N={int(n_metar_val)}) uyuşmuyor. İki değer aynı olmalıdır.')
