...
# --- HEDEF KLASÖRÜ DİNAMİK OLARAK BELİRLE (DİĞER PC'LER İÇİN) ---
HEDEF_KLASOR = os.path.join(os.path.expanduser("~"), "Desktop", "check")
ARSIV_KLASORU = os.path.join(HEDEF_KLASOR, "Arsiv")

# Eğer klasörler mevcut değilse otomatik olarak oluştur
try:
...
def arayuzde_goster(birlesik, hatali_kayitlar, sinoptik_sayisi, metar_normal_sayisi, speci_sayisi, ay, yil):
    ...
    def ekrandakileri_excele_aktar():
        ...
        try:
            ...
            with pd.ExcelWriter(dosya_yolu, engine='xlsxwriter') as writer:
                ...
                worksheet.set_row(0, 30) # Başlık satırı yüksekliği
                ...
                worksheet.set_column(3, 3, 35) # Açıklama
                ...
                # Yazdırma ayarları: Yatay A4, kenar boşlukları dar
                worksheet.set_paper(9) # 9 = A4 paper
                worksheet.set_landscape()
                worksheet.set_margins(left=0.3, right=0.3, top=0.5, bottom=0.5)
                worksheet.fit_to_pages(1, 0) # Genişliği 1 sayfaya sığdır
                worksheet.repeat_rows(0) # Her sayfada üst başlığı tekrarla
                
                # Sayfa düzeni görünümü (isteğe bağlı, normal görünüm daha iyidir)
    ...
    cols = ("Seç", "Tarih", "Saat", "SİNOPTİK Saat", "METAR Saat", "Hata Kodu", "Açıklama", "Hatalı Kod", "Tavsiye Kod", "SİNOPTİK Şifresi", "METAR Şifresi", "Aksiyon")
    tree = ttk.Treeview(tree_frame, columns=cols, show="headings", style="Treeview", displaycolumns=("Seç", "Tarih", "SİNOPTİK Saat", "METAR Saat", "Hata Kodu", "Hatalı Kod", "Tavsiye Kod", "Aksiyon"))
...
def aylik_rapor_olustur(run_async=True, load_from_cache=False):
    ...
    def islem_yurut(load_from_cache=False):
        ...
        try:
            if load_from_cache:
                ...
                if os.path.exists(cache_path):
                    ...
                    safe_showerror("Hata", "Daha önce kaydedilmiş bir analiz bulunamadı. Lütfen önce normal bir analiz çalıştırın.")
                    # UI temizle
                    def finalize_ui():
            ...
            sadece_en_yeni_dosyalari_tut()
            update_overall_progress(10, "Eski geçici dosyalar temizlendi.")
...
# --- YENİ EKLENEN: HIZLI METAR CACHE SİSTEMİ ---
            metar_cache = {}
            ...
            birlesik = dm2.hata_analizi_yap(birlesik, df_metar, progress_callback=progress_cb)
            ...
            bulten_list = birlesik[b_col].astype(str).tolist()
            ...
            for i, row in enumerate(b_records):
                ...
                if MetarDecoder is not None and metar_str and len(metar_str.strip()) >= 10:
                    try:
                        ...
                        add_error(i, "h363", f"METAR Çözümleme Hatası: Beklenmeyen format veya karakter ({e})")
...
# --- METAR HALİHAZIR HAVA (1, 2 ve 3. GRUP) SÜTUNLARINI BİRLEŞTİR ---
    ...
    if run_async and not console_mode:
        threading.Thread(target=lambda: islem_yurut(load_from_cache), daemon=True).start()
    else:
        islem_yurut(load_from_cache)
...
if not console_mode:
    ...
    def dosyalari_indir_ac():
        ...
        indirme_yili = simpledialog.askinteger('Dosya Adının Yılı', 'Kardelen\'den dosyaları indirirken kaydedeceğiniz dosyanın YILINI giriniz:\n(Örn: 2026)', initialvalue=v_yil, minvalue=2000, maxvalue=2050, parent=root)
        ...
        messagebox.showinfo('Dosya İsimlendirme', f'Kardelen açıldığında dosyaları tam olarak şu isimlerle indirmelisiniz:\n\nSİNOPTİK: {indirme_ayi:02d}{indirme_yili}-sinoptik.xls\nMETAR: {indirme_ayi:02d}{indirme_yili}-metar.xls', parent=root)
        ...
        def arkaplanda_tarayici_ac():
            try:
                # DİNAMİK YOL: Programın her bilgisayarda çalışabilmesi için sabit yol yerine kullanıcının masaüstünü otomatik bul.
                hedef_klasor = HEDEF_KLASOR
                if not os.path.exists(hedef_klasor):
                    os.makedirs(hedef_klasor)
    ...
    # ARŞİV SÜTUNLARINI GÜNCELLE butonu kaldırıldı
