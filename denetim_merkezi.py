...
def dosya_oku_html_veya_excel(dosya_yolu, header_satiri):
    ...
    if not tum_kayitlar:
        raise ValueError("Dosyadan hiç tablo okunamadı (HTML/Excel değil)")
        ...
        try:
            ...
            if not df_sin.empty:
                def raw_rasat_olustur(row):
                    ...
                    # Meteorolojik Sıralama (Mantıksal Akış)
                    oncelik_sirasi = [
                        'ir', 'ix', 'h', 'vv', 'n', 'dd', 'ff', # Ana Grup
                        't', 'td', 'p0', 'p', 'a', 'ppp',       # Sıcaklık/Basınç
                        'rrr', 'tr', 'ww', 'w1', 'w2',          # Yağış/Hadise
                        'nh', 'cl', 'cm', 'ch',                 # Bulutlar
                        'tx', 'tn', 'tg', 'e'                   # Ekstra
                    ]
                    
                    # 1. Önce sıralı olanları ekle
                    for col in oncelik_sirasi:
                        if col in df_sin.columns:
                            ...
                            if pd.notna(val):
                                ...
                                if val_str:
                                    items.append(f"{col.upper()}:{val_str}")

                    # 2. Listede olmayan diğer sütunları ekle
                    for col in df_sin.columns:
                        if col not in exclude and col not in oncelik_sirasi:
                            val = row.get(col)
                            if pd.notna(val):
                                val_str = str(val).strip()
                                if val_str.endswith(".0"): val_str = val_str[:-2]
