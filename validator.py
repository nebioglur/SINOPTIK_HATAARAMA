...
class WeatherLogValidator:
    def __init__(self, sin_row, met_row, metar_gmt=None, sinoptik_gmt=None):
        ...
        if 'Bulten' in self.met and pd.notna(self.met['Bulten']):
            b_str = str(self.met['Bulten'])
            # Daha güvenli temizlik: Büyük/küçük harf veya İ/I farkından etkilenmemesi için Regex kullanıyoruz
            b_str = re.split(r'İLG[İI]L[İI] METAR GEÇM[İI]Ş[İI]|ILGILI METAR GECMISI', b_str, flags=re.IGNORECASE)[0]
    ...
    def check_visibility_and_fog(self):
        ...
        if pd.notna(vis_met):
            ...
            if vis_met < 1000:
                ...
                hadise_var = any(kod in hadise_met_clean for kod in kisitlayici_kodlar)
    ...
    def check_thunderstorm_and_hail(self):
        ...
        if pd.notna(ww_sin):
            ...
            if 91 <= ww <= 99 and not has_ts:
                self.errors.append({"kod": "h286", "mesaj": f"Sinoptik ww={ww} (Oraj) kodlanmış ancak METAR'da Oraj (TS) raporlanmamış."})
                
            # 2. Dolusuz Oraj (95, 97) ama METAR'da Dolu (GR/GS) var
            if ww in [95, 97] and has_hail:
                self.errors.append({"kod": "h287", "mesaj": f"Sinoptik ww={ww} (Dolusuz Oraj) kodlanmış ancak METAR'da Dolu (GR/GS) raporlanmış."})
                
            # 3. Dolulu Oraj (96, 99) ama METAR'da Dolu yok
            if ww in [96, 99] and not has_hail:
                self.errors.append({"kod": "h288", "mesaj": f"Sinoptik ww={ww} (Dolulu Oraj) kodlanmış ancak METAR'da Dolu (GR/GS) raporlanmamış."})

            # 4. METAR'da hem TS hem Dolu varsa, Sinoptik kod buna uygun (Örn: 90, 93, 94, 96, 99) olmalıdır
            if has_ts and has_hail:
                if ww not in [90, 93, 94, 96, 99]:
                    self.errors.append({"kod": "h289", "mesaj": f"METAR'da Oraj ve Dolu (TS + GR/GS) var, ancak Sinoptik ww={ww} kodu uyumsuz (90, 93, 94, 96 veya 99 beklenir)."})
    ...
    def check_hail_size(self):
        ...
        if pd.notna(ww_sin):
            try:
                if int(ww_sin) in [87, 88, 89, 90, 93, 94, 96, 99]: has_hail_sin = True
    ...
    def check_specific_weather_phenomena(self):
        ...
        if pd.notna(ww_sin):
            ...
            if has_fz and ww not in [56, 57, 66, 67]:
                self.errors.append({"kod": "h364", "mesaj": f"METAR'da Donduran Yağış/Çisenti (FZRA/FZDZ) var, ancak Sinoptik ww={ww} uyumsuz (56, 57, 66 veya 67 beklenir)."})
            elif ww in [56, 57, 66, 67] and not has_fz:
                self.errors.append({"kod": "h364", "mesaj": f"Sinoptik ww={ww} (Donduran Yağış/Çisenti) kodlanmış ancak METAR'da FZRA/FZDZ raporlanmamış."})
                
            kar_kodlari = list(range(70, 80)) + [68, 69, 83, 84, 85, 86]
            if has_sn and ww not in kar_kodlari:
                self.errors.append({"kod": "h365", "mesaj": f"METAR'da Kar (SN) raporlanmış, ancak Sinoptik ww={ww} kodu uyumsuz (70-79, 83-86 veya 68-69 beklenir)."})
            elif ww in [70, 71, 72, 73, 74, 75, 85, 86] and not has_sn:
                self.errors.append({"kod": "h365", "mesaj": f"Sinoptik ww={ww} (Kar Yağışı) kodlanmış ancak METAR'da Kar (SN) raporlanmamış."})
                
            if has_br:
                # METAR'da BR (Pus) var ise Sinoptik'te ww'nin en az 10 (Pus) veya daha öncelikli bir hadise (Yağış, Sis, Oraj, RERA vb. ww>=10) olması gerekir.
                if ww < 10:
                    self.errors.append({"kod": "h366", "mesaj": f"METAR'da Pus (BR) var, ancak Sinoptik ww={ww} uyumsuz (10 veya daha yüksek öncelikli bir hadise beklenir)."})
            elif ww == 10 and not has_br:
                if pd.isna(vis_met) or vis_met <= 5000:
                    self.errors.append({"kod": "h366", "mesaj": f"Sinoptik ww=10 (Pus) kodlanmış ancak METAR'da Pus (BR) raporlanmamış (Görüş <= 5000m iken beklenir)."})
                
            if has_fu_va and ww != 4:
                self.errors.append({"kod": "h367", "mesaj": f"METAR'da Duman/Volkanik Kül (FU/VA) var, ancak Sinoptik ww={ww} uyumsuz (04 beklenir)."})
            elif ww == 4 and not has_fu_va:
                if pd.isna(vis_met) or vis_met <= 5000:
                    self.errors.append({"kod": "h367", "mesaj": f"Sinoptik ww=04 (Duman/Volkanik Kül) kodlanmış ancak METAR'da FU veya VA raporlanmamış (Görüş <= 5000m iken beklenir)."})
                
            if has_hz and ww != 5:
                self.errors.append({"kod": "h368", "mesaj": f"METAR'da Toz Pusu (HZ) var, ancak Sinoptik ww={ww} uyumsuz (05 beklenir)."})
            elif ww == 5 and not has_hz:
                if pd.isna(vis_met) or vis_met <= 5000:
                    self.errors.append({"kod": "h368", "mesaj": f"Sinoptik ww=05 (Toz Pusu) kodlanmış ancak METAR'da Toz Pusu (HZ) raporlanmamış (Görüş <= 5000m iken beklenir)."})

            if has_ss_ds and not (30 <= ww <= 35):
                self.errors.append({"kod": "h369", "mesaj": f"METAR'da Kum/Toz Fırtınası (SS/DS) var, ancak Sinoptik ww={ww} uyumsuz (30-35 beklenir)."})
            elif (30 <= ww <= 35) and not has_ss_ds:
                self.errors.append({"kod": "h369", "mesaj": f"Sinoptik ww={ww} (Kum/Toz Fırtınası) kodlanmış ancak METAR'da SS/DS raporlanmamış."})
                
            if has_po and ww != 8:
                self.errors.append({"kod": "h370", "mesaj": f"METAR'da Toz Şeytanı (PO) var, ancak Sinoptik ww={ww} uyumsuz (08 beklenir)."})
            elif ww == 8 and not has_po:
                self.errors.append({"kod": "h370", "mesaj": f"Sinoptik ww=08 (Toz Şeytanı) kodlanmış ancak METAR'da Toz Şeytanı (PO) raporlanmamış."})

            if has_sh and not (80 <= ww <= 90):
                self.errors.append({"kod": "h371", "mesaj": f"METAR'da Sağanak Yağış (SH..) var, ancak Sinoptik ww={ww} uyumsuz (80-90 arası beklenir)."})
            elif (80 <= ww <= 90) and not (has_sh or self._has_active_wx_match(r'TS')):
                self.errors.append({"kod": "h371", "mesaj": f"Sinoptik ww={ww} (Sağanak Yağış) kodlanmış ancak METAR'da Sağanak (SH) raporlanmamış."})

            if has_blsn_drsn and not (36 <= ww <= 39):
                self.errors.append({"kod": "h372", "mesaj": f"METAR'da Sürüklenen/Savrulan Kar (BLSN/DRSN) var, ancak Sinoptik ww={ww} uyumsuz (36-39 arası beklenir)."})
            elif (36 <= ww <= 39) and not has_blsn_drsn:
                self.errors.append({"kod": "h372", "mesaj": f"Sinoptik ww={ww} (Sürüklenen/Savrulan Kar) kodlanmış ancak METAR'da BLSN veya DRSN raporlanmamış."})
                
            if has_blsn_drsn or (36 <= ww <= 39):
                ff_sin = self.sin.get('ff')
                if ff_sin is not None and ff_sin < 3:
                    self.errors.append({"kod": "VAL_MANTIK_BLSN", "mesaj": f"Kar Savrulması (BLSN/DRSN veya ww={ww}) raporlanmış ancak rüzgar hızı çok düşük (Sinoptik ff={ff_sin} m/s). Savrulma için genelde >=3 m/s rüzgar beklenir."})
    ...
    def check_sky_obscured_logic(self):
        ...
        if pd.notna(n_sin) and n_sin == 9:
            ...
            if pd.notna(ww_sin):
                try:
                    ww = int(ww_sin)
                    if (40 <= ww <= 49) or (70 <= ww <= 79) or (30 <= ww <= 39) or ww in [4, 5, 8]:
        ...
        if SynopDecoder is not None and pd.notna(rasat_sin) and len(str(rasat_sin).strip()) > 10:
            if ":" not in str(rasat_sin): # Manuel birleştirme değil, gerçek raw format ise
                ...
                if s_data:
                    ...
                    s_nh = self.sin.get('nh_sin', self.sin.get('N')) # Bazen Nh yerine N giriliyor
                    ...
                    if dec_nh is not None and pd.notna(s_nh) and float(s_nh) != float(dec_nh):
                    ...
                    if dec_cl is not None and pd.notna(s_cl) and float(s_cl) != float(dec_cl):
                    ...
                    if dec_cm is not None and pd.notna(s_cm) and float(s_cm) != float(dec_cm):
                    ...
                    if dec_ch is not None and pd.notna(s_ch) and float(s_ch) != float(dec_ch):
